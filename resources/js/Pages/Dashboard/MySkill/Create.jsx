...
function Create({subject,skills}) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('my-skill.store'));
    };
    return (
        <AuthenticatedLayout
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight"><Link href={route('my-skill.index')}>{subject}</Link> {'->'} create</h2>}
        >
            <Head title={subject+' Create'} />
            <div className="w-9/12 mx-auto py-12">
                <div className={'w-full my-4 px-1'}>
                    <Link href={route('my-skill.index')} className={'px-5 py-2 bg-gray-800 text-white rounded-2xl'}> {'<'} {subject}s</Link>
                </div>
